...
// 驱动支持的设备
struct pci_device_id XofDeviceId[] = {
    {XOF_VENDOR_ID, XOF_DEVICE_ID, XOF_VENDOR_ID, XOF_SUBDEVICE_ID, 0, 0, 0},
    {XOF_VENDOR_ID, XOF_GS1002F_V3_DEVICE_ID, XOF_VENDOR_ID, XOF_GS1002F_V3_SUBDEVICE_ID, 0, 0, 0},
    {XOF_VENDOR_ID, XOF_GS1004F_DEVICE_ID, XOF_VENDOR_ID, XOF_GS1004F_SUBDEVICE_ID, 0, 0, 0},
    {XOF_VENDOR_ID, XOF_GS1004F_V3_DEVICE_ID, XOF_VENDOR_ID, XOF_GS1004F_V3_SUBDEVICE_ID, 0, 0, 0},
    {XOF_VENDOR_ID, XOF_GS1104F_DEVICE_ID, XOF_VENDOR_ID, XOF_GS1104F_SUBDEVICE_ID, 0, 0, 0},
    {XOF_VENDOR_ID, XOF_GQ1001_DEVICE_ID, XOF_VENDOR_ID, XOF_GQ1001_SUBDEVICE_ID, 0, 0, 0},
    {XOF_VENDOR_ID, XOF_GQ1001_NEW_DEVICE_ID, XOF_VENDOR_ID, XOF_GQ1001_NEW_SUBDEVICE_ID, 0, 0, 0},
    {0,}
};

MODULE_DEVICE_TABLE(pci, XofDeviceId);

struct pci_driver XofDriver = {
    .name = MV_MODULE_LONG_NAME,
    .id_table = XofDeviceId,
    .probe = ModuleProbe,
    .remove = ModuleRemove,
...
int MV_AddDevice(void* pPciDevice, char* chDeviceName, char* chXofClassName)
{
    ...
    pstPciDevice->pstClass = class_create(THIS_MODULE, (const char *)chXofClassName);
#else
	pstPciDevice->pstClass = class_create((const char *)chXofClassName);
#endif
...
int MV_RequestIrq(void* pPciDevice, irqreturn_t (*interruptfn)(int irq, void *data), void* pDriverData)
{
    ...
    do
    {
        ...
        {
            nRet = request_irq(pstDeviceData->hMSInterrupt[nIrqCount], interruptfn, IRQF_SHARED, MV_MODULE_LONG_NAME, pDriverData);
...
int ModuleSuspend(struct pci_dev *dev, pm_message_t state)
{
    ...
    PPCI_DEVICE pstPciDevice = dev->dev.driver_data;
    if (NULL == pstPciDevice)
    {
        return -EINVAL;
    }
...
static int InitModule(void)
{
    ...
    // 注册pci driver
    nRet = pci_register_driver(&XofDriver);
    if (nRet < 0) {
...
static void ExitModule(void)  
{
    ...
    pci_unregister_driver(&XofDriver);
    printk(KERN_INFO"[%s:%d]:end\n", __FUNCTION__, __LINE__);
}
