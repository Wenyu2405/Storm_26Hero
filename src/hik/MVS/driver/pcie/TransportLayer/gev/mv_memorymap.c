...
int MV_GeKernelPages(MV_MEMORYPAGELIST* pMap, void* pKernelAddress, unsigned int nSize )
{

    struct page** lPageList     = NULL;
    unsigned long nReqPageCount = 0;

    if (NULL == pMap || NULL == pKernelAddress)
    {
        return -1;
    }

    pMap->KernelAddress = pKernelAddress;
    nReqPageCount = MV_GetPageCnt(nSize);
    // nReqPageCount = lLastPage - lFirstPage + 1;
    ...
    unsigned int i = 0;
    for(i = 0; i < nReqPageCount; ++i)
    {
        lPageList[i] = MV_VirtToPage(pKernelAddress + i * MV_PageSize());
    }

    pMap->PageList = (void*)lPageList;
    pMap->PageCount = nReqPageCount;
    pMap->KernelAddress = pKernelAddress;
    pMap->Size = nSize;
    pMap->OffsetPage = 0;

    pMap->nUnSgMapPageCount = nReqPageCount;
    pMap->nSgMapPageCount = 0;
    pMap->nNeedMapPageCount = 0;
    pMap->nUnSgMapSize = nSize;
    pMap->nSgMapSize = 0;
    return 0;
}


int MV_ReleaseKernelPages(MV_MEMORYPAGELIST* pMap)
{
    unsigned int i          = 0;
    struct page* lPage      = NULL;
    struct page** lPageList = ( struct page** ) pMap->PageList;
...
inline unsigned int MV_GetPageCnt(unsigned int nSize)
{
    return  DIV_ROUND_UP(nSize, PAGE_SIZE);
}

inline void*  MV_VirtToPage(void* pVirBuffer)
{
    return virt_to_page(pVirBuffer);
}

inline void MV_GetPage(void* pstPage)
{
    return get_page((struct page*)pstPage);
}

inline void* MV_VMallocToPage(void* pVirBuffer)
{
    return vmalloc_to_page(pVirBuffer);
}
