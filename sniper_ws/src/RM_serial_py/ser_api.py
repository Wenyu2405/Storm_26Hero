...
# def build_data_video_packet(payload_150bytes):
#     """
#     构建 0x0310 图传链路自定义数据的 data 段。
#     payload_150bytes: bytes 或 bytearray，长度应为 150。
#     不足 150 字节会补零，超出会截断。
#     """
#     data = bytearray(150)
#     actual = bytes(payload_150bytes)[:150]
...
def build_data_video_fragment(frame_id, frag_idx, frag_total, payload):
    """
    按 [帧ID:2B][分片序号:1B][总分片数:1B][数据长度:2B][载荷:≤294B] 打包单个分片。

    参数:
        frame_id   (int): 帧编号, 0~65535, 用于接收端判断是否属于同一帧
        frag_idx   (int): 当前分片序号, 从 0 开始
        frag_total (int): 该帧的总分片数
        payload  (bytes): 本分片的实际载荷, 长度 ≤ 294

    返回:
        bytearray: 6 + len(payload) 字节的完整分片数据段
    """
    if len(payload) > 294:
        raise ValueError(f"单个分片载荷不能超过 294 字节, 当前 {len(payload)}")

    header = struct.pack('<HBBh',
        frame_id & 0xFFFF,      # 帧 ID,        小端 uint16
        frag_idx & 0xFF,        # 分片序号,     uint8
        frag_total & 0xFF,      # 总分片数,     uint8
        len(payload) & 0xFFFF   # 数据长度,     小端 uint16
    )
    # header 长度 = 2+1+1+2 = 6 字节
    return bytearray(header) + bytearray(payload)


# ======================== 整帧分片 ========================

MAX_FRAG_PAYLOAD = 294  # 每个分片最大载荷

def fragment_frame(frame_id, frame_data):
    """
    将一整帧数据拆分为多个分片, 每个分片载荷 ≤ 294 字节。

    参数:
        frame_id   (int): 帧编号
        frame_data (bytes): 完整帧数据（如一个 JPEG 压缩后的字节流）

    返回:
        list[bytearray]: 每个元素是一个打包好的分片（含 6B 头 + 载荷）
    """
    total_len = len(frame_data)
    # 向上取整计算总分片数
    frag_total = (total_len + MAX_FRAG_PAYLOAD - 1) // MAX_FRAG_PAYLOAD
    if frag_total == 0:
        frag_total = 1  # 空帧也发一个空分片

    fragments = []
    for i in range(frag_total):
        start = i * MAX_FRAG_PAYLOAD
        end = min(start + MAX_FRAG_PAYLOAD, total_len)
        chunk = frame_data[start:end]
        frag = build_data_video_fragment(frame_id, i, frag_total, chunk)
