...
void MultiLightCtrlDemo::ImageCallBackInner(unsigned char * pData, MV_FRAME_OUT_INFO_EX* pFrameInfo)
{
    ...
    if(pFrameInfo->nSubImageNum > 0)
    {
        if(pFrameInfo->nSubImageNum <= EXPOSURE_NUM)
        {
            for (unsigned int i=0;i<pFrameInfo->nSubImageNum;i++)
            {
                stDisplayInfo.pImageBuf = pFrameInfo->SubImageList.pstSubImage[i].pImageBuf;
                stDisplayInfo.nImageLen = pFrameInfo->SubImageList.pstSubImage[i].nImageLen;
                stDisplayInfo.nWidth = pFrameInfo->SubImageList.pstSubImage[i].nWidth;
                stDisplayInfo.nHeight = pFrameInfo->SubImageList.pstSubImage[i].nHeight;
                stDisplayInfo.enPixelType = pFrameInfo->SubImageList.pstSubImage[i].enPixelType;
        ...
        {
            printf("ERROR:sub image number[%d] is larger than light number[%d]",pFrameInfo->nSubImageNum,EXPOSURE_NUM);
            return;
        }
    }
    else //need to reconstruct image
    {
        ...
        {
            stImgReconstructionParam.nWidth = pFrameInfo->nExtendWidth;
            stImgReconstructionParam.nHeight = pFrameInfo->nExtendHeight;
            stImgReconstructionParam.enPixelType = pFrameInfo->enPixelType;
            stImgReconstructionParam.pSrcData = pData;
            stImgReconstructionParam.nSrcDataLen = pFrameInfo->nFrameLenEx;
        ...
        else
        {
            ...
            {
                stDisplayInfo.pImageBuf = pData;
                stDisplayInfo.nImageLen = pFrameInfo->nFrameLenEx;
                stDisplayInfo.nWidth = pFrameInfo->nExtendWidth;
                stDisplayInfo.nHeight = pFrameInfo->nExtendHeight;
