...
-> 直接包进 0x0310 -> 图传发送端串口 (921600)
0x0310 属于图传链路，不需要交互头，data 直接是裸数据
"""
...
class VideoSerialBridge(Node):
    def __init__(self):
        ...
        self.declare_parameter('serial_baud', 921600)       # 图传链路波特率
        ...
        self.get_logger().info(
            f'桥接启动: {topic} (150B) -> 0x0310 -> {serial_port} @ {serial_baud}'
        )
    ...
    def callback(self, msg):
        ...
        # 0x0310 图传链路：data 直接是裸数据，不需要交互头
        data = bytes(msg.data)
