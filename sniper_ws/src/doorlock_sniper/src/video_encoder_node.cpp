...
#include <cctype>
#include <chrono>
#include <cstring>  // ä¸º memcpy/memset
#include <filesystem>
#include <iomanip>
#include <sstream>
...
: Node("video_encoder_node", options),
  ...
  appsink_(nullptr),
  bus_(nullptr),
  packet_sequence_id_(0),    // åˆå§‹åŒ–é¡ºåºä¸Žå£°æ˜Žä¸€è‡´
  frame_count_(0),
  display_running_(false)    // æœ€åŽåˆå§‹åŒ–
{
...
void VideoEncoderNode::initialize_gstreamer()
{
  ...
  if (low_bitrate_mode) {
    g_object_set(
      ...
      "byte-stream", TRUE,
      "key-int-max", key_int,     // å‡å°‘ I å¸§å¼€é”€
      "bframes", 4,
      "rc-lookahead", 40,
      "sync-lookahead", 20,
  ...
  // ç¡®ä¿ä¸‹æ¸¸çœ‹åˆ°å¯æµå¼é‡ç»„çš„ Annex-B å­—èŠ‚æµï¼Œå¹¶å‘¨æœŸé‡å¤ SPS/PPS
  g_object_set(
    G_OBJECT(parser),
    "config-interval", -1,
  ...
  cv::Mat * static_removed)
{
  ...
  // ä¸­å¿ƒåŒºåŸŸä¿æŠ¤ï¼šä¸åšé™æ€æ¨¡ç³Š
  ...
  // è¿åŠ¨æ‹–å½±ï¼šç®€å•æ—¶åŸŸ maxï¼ˆå½“å‰+åŽ†å²Nå¸§ï¼‰ï¼Œä»…ä½œç”¨åœ¨è¿åŠ¨åŒºåŸŸè”åˆæŽ©ç 
  if (param_motion_trail_frames_ > 0) {
...
{
  try {
    // ä»…åœ¨æ˜Žç¡®è¦æ±‚ä½ŽäºŽ 60fps æ—¶åšæŠ½å¸§ï¼›60fps æ¨¡å¼ä¸ä¸»åŠ¨ä¸¢å¸§
    if (param_output_fps_ < 60) {
...
// 150B åˆ†åŒ… + å¸¦å®½çª—å£é™é€Ÿ + é˜Ÿåˆ—æ—¶å»¶ä¸Šé™
void VideoEncoderNode::pull_stream_and_packetize()
{
  ...
  while (true) {
    ...
    if (gst_buffer_map(buffer, &map, GST_MAP_READ)) {
      ...
      // 2ç§’æ»‘åŠ¨çª—å£ç¡¬é™é€Ÿï¼šä»»ä½•çª—å£å†…æ€»å­—èŠ‚ä¸è¶…è¿‡ window_limit_bytes
      ...
      // æŽ’é˜Ÿæ—¶å»¶ä¸Šé™ï¼šé˜²æ­¢çªå‘é€ æˆé•¿å»¶æ—¶ã€‚è¶…é™æ—¶ä¸¢å¼ƒæ—§æ•°æ®ã€‚
      if (stream_buffer_.size() > max_backlog_bytes) {
        ...
        // å°½é‡å¯¹é½åˆ°ä¸‹ä¸€ä¸ª Annex-B èµ·å§‹ç ï¼Œå‡å°‘è§£ç é”™è¯¯æŒç»­æ—¶é—´
