...
int MV_GetUserPages( long* aContext, MV_MEMORYPAGELIST* aMap, void* aUserAddress, uint64_t aSize )
{
    ...
    unsigned int 	i = 0;
    uint64_t 		lFirstPage = 0;
    uint64_t 		lLastPage = 0;
    uint64_t 		nReqPageCount = 0;
    ...
    lFirstPage = ( ( uint64_t ) aUserAddress ) >> PAGE_SHIFT;
    lLastPage = ( ( ( uint64_t ) aUserAddress ) + ( uint64_t ) aSize ) >> PAGE_SHIFT;
    if (( ( ( uint64_t ) aUserAddress ) + ( uint64_t ) aSize ) % PAGE_SIZE > 0)
    {
    ...
    lPageList = (struct page**)vmalloc( sizeof( struct page* ) * nReqPageCount);
    if( !lPageList )
    {
        printk(KERN_ERR "kmalloc failed, req page count [%llu]", nReqPageCount);
        return -1;
    }
    ...
    if (lPageCount <= 0)
    {
        printk(KERN_ERR "get_user_pages_fast failed, lPageCount [%u], nReqPageCount[%llu]", lPageCount, nReqPageCount);
        return -1;
    }
...
//     {
//         printk("XofDriver: vma is too big\n");
//         return -1;
//     }
...
//         if (0 != nRet)
//         {
//             printk("XofDriver: remap_page_range failed ,nRet[%d], i[%d]\n", nRet, i);
//             return -1;
//         }
